...
namespace SkinningSample
{
    ...
    public class SkinningSampleGame : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            animationPlayer.Update(gameTime.ElapsedGameTime, true, Matrix.Identity * Matrix.CreateScale(2.1f)*Matrix.CreateTranslation(105,-27,60));
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            Matrix view = Matrix.CreateTranslation(-100, -40, 0) * 
            ...
            Matrix[] transforms = new Matrix[table.Bones.Count];
            table.CopyAbsoluteBoneTransformsTo(transforms);
            ...
            foreach (ModelMesh mesh in table.Meshes)
            {
                ...
                foreach (BasicEffect effect in mesh.Effects)
                {
                    ...
                    effect.World = transforms[mesh.ParentBone.Index] *
                         Matrix.CreateTranslation(new Vector3(100, 0, 0));
                    effect.View = view;
            ...
            Matrix[] transform = new Matrix[chair.Bones.Count];
            chair.CopyAbsoluteBoneTransformsTo(transforms);
            ...
            foreach (ModelMesh mesh in chair.Meshes)
            {
                ...
                foreach (BasicEffect effect in mesh.Effects)
                {
                    ...
                    Plane a;
                    a.D = -50;
                    a.Normal = new Vector3(1, 0, 0);
                    Matrix.CreateReflection(a);
                    effect.World = transforms[mesh.ParentBone.Index] *
                         Matrix.CreateTranslation(new Vector3(-75, 0, 130)) * Matrix.CreateScale(0.8f) * Matrix.CreateRotationY(MathHelper.PiOver2);
                    effect.View = view;
                    effect.Projection = Matrix.CreatePerspectiveFieldOfView(
                        MathHelper.ToRadians(45.0f), aspectRatio,
                        1.0f, 10000.0f);
            ...
            transform = new Matrix[chair2.Bones.Count];
            chair2.CopyAbsoluteBoneTransformsTo(transform);

            // Draw the model. A model can have multiple meshes, so loop.
            foreach (ModelMesh mesh in chair2.Meshes)
            {
                // This is where the mesh orientation is set, as well 
                // as our camera and projection.
                foreach (BasicEffect effect in mesh.Effects)
                {
                    effect.EnableDefaultLighting();
                    effect.World = transforms[mesh.ParentBone.Index] *
                         Matrix.CreateTranslation(new Vector3(-70, 0, -130)) * Matrix.CreateScale(0.8f) * Matrix.CreateRotationY(-MathHelper.PiOver2);
                    effect.View = view;
                    effect.Projection = Matrix.CreatePerspectiveFieldOfView(
                        MathHelper.ToRadians(45.0f), aspectRatio,
                        1.0f, 10000.0f);
